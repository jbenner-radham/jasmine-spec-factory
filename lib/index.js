...
module.exports = function () {
    ...
    if (!ast.hasPublicMembersDeclaration()) {
        console.error(chalk.red('The AMD module you are parsing does not appear to have a `publicMembers` object.'));
        console.error(chalk.red(`If you believe this to be an error, please report it at: <${chalk.bold.underline(pkg.bugs.url)}>.`));
        process.exit(1);
    }
    ...
    try {
        ...
        returnsUndefinedPartial = templates.shift();
    ...
    let methods        = publicMethodNames.map(methodName => {
        ...
        if (method.name === 'bindEvents' || method.name === 'listen') {
            method.returnsUndefined = true;
        }
    ...
    Handlebars.registerPartial('returnsUndefined', returnsUndefinedPartial);
